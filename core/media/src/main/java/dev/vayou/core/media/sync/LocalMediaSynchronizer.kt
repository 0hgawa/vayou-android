...
import dev.vayou.core.common.extensions.VIDEO_COLLECTION_URI
import dev.vayou.core.common.extensions.getStorageVolumes
...
import dev.vayou.core.media.model.MediaVideo
import java.io.File
...
class LocalMediaSynchronizer @Inject constructor(
    ...
    @Dispatcher(VayouDispatchers.IO) private val dispatcher: CoroutineDispatcher,
) : MediaSynchronizer {
    ...
    private suspend fun updateMedia(media: List<MediaVideo>) = withContext(Dispatchers.Default) {
        ...
        unwantedMedia.forEach { media ->
            try {
                ...
                Log.w(TAG, "sync: cleanup failed", e)
            }
        }
    ...
    companion object {
        private const val TAG = "MediaSynchronizer"

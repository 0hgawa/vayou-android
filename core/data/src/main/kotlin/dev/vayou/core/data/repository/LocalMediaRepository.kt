...
class LocalMediaRepository @Inject constructor(
    ...
    override fun getVideosFlowFromFolderPath(folderPath: String): Flow<List<Video>> =
        mediumDao.getAllWithInfoFromDirectory(folderPath).map {
            it.map(MediumWithInfo::toVideo)
        }

    override fun getFoldersFlow(): Flow<List<Folder>> = directoryDao.getAllWithMedia().map {
    ...
    override suspend fun addExternalSubtitleToMedium(uri: String, subtitleUri: Uri) {
        ...
        mediumStateDao.upsert(
            stateEntity.copy(externalSubs = newExternalSubs, lastPlayedTime = System.currentTimeMillis()),
        )
    }

...
class SearchMediaUseCase @Inject constructor(
    ...
    operator fun invoke(query: String): Flow<SearchResults> {
        ...
        return combine(
            ...
            val scoredFolders = folders.asSequence().mapNotNull { folder ->
            ...
            val scoredVideos = videos.asSequence().mapNotNull { video ->
...
private class SearchMatcher(query: String) {
    ...
    private fun Char.isWordBoundary(): Boolean {
        return this in WORD_BOUNDARIES
    }

    companion object {
        private val WORD_BOUNDARIES = setOf(' ', '_', '-', '.', '/', '\\', '[', ']', '(', ')')
    }
}
